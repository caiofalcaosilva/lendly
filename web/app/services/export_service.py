...
def _report_dict(r: Report) -> dict[str, Any]:
    return {
        "id": str(r.id),
        "target": {"type": "item", "id": str(r.item.id), "title": r.item.title}
        if r.item
        else {
            "type": "user",
            "id": str(r.reported_user.id),
...
def export_user_data(current_user: User) -> dict[str, Any]:
    ...
    favorites = Item.objects(
        id__in=[f.id for f in (current_user.favorites or [])], is_active=True
    )
    ...
    return {
        ...
        "items": [
            ...
            for i in item_service.get_user_items(str(current_user.id), current_user)
        ],
        "loan_requests": {
            "sent": [
                r.model_dump(mode="json")
                for r in loan_request_service.get_sent_requests(current_user)
            ],
            "received": [
                r.model_dump(mode="json")
                for r in loan_request_service.get_received_requests(current_user)
            ],
        },
        "reviews": {
            "given": [
                {
                    "id": str(r.id),
                    "item_title": r.loan_request.item.title,
                    "reviewed_name": r.reviewed.name,
                    "rating": r.rating,
                    "comment": r.comment,
                    "created_at": r.created_at.isoformat(),
                }
