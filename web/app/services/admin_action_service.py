...
def get_admin_actions(limit: int = 50) -> List[AdminActionEntry]:
    ...
    reports = Report.objects(status__in=["dismissed", "actioned"], reviewed_by__ne=None).order_by(
        "-reviewed_at"
    ).limit(_SOURCE_LIMIT)
    for r in reports:
        target_label = r.item.title if r.item else (r.reported_user.name if r.reported_user else "—")
        target_id = str(r.item.id) if r.item else (str(r.reported_user.id) if r.reported_user else "")
        entries.append(AdminActionEntry(
            action_type="report_actioned" if r.status == "actioned" else "report_dismissed",
    ...
    verifications = VerificationSubmission.objects(
        status__in=["approved", "rejected"], reviewed_by__ne=None
    ).order_by("-reviewed_at").limit(_SOURCE_LIMIT)
    for v in verifications:
        entries.append(AdminActionEntry(
            action_type="verification_approved" if v.status == "approved" else "verification_rejected",
    ...
    changed_users = User.objects(status_changed_by__ne=None).order_by("-status_changed_at").limit(_SOURCE_LIMIT)
    ...
    changed_items = Item.objects(status_changed_by__ne=None).order_by("-status_changed_at").limit(_SOURCE_LIMIT)
    ...
    admin_changed_users = User.objects(admin_status_changed_by__ne=None).order_by(
