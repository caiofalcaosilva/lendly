  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Todos os grupos</h1>
      </div>
      <p className="text-gray-500 dark:text-gray-400 text-sm mb-8">
        Todo grupo da plataforma, mesmo os que você não participa.
      </p>

      {loading ? (
        <div className="flex justify-center py-12"><Spinner className="w-8 h-8 text-green-600" /></div>
      ) : groups.length === 0 ? (
        <EmptyState icon={UsersRound} title="Nenhum grupo cadastrado ainda" />
      ) : (
