...
export default function AdminItemsPage() {
  ...
  const runBulkAction = async (action: 'activate' | 'deactivate') => {
    ...
    if (!window.confirm(`${action === 'activate' ? 'Ativar' : 'Desativar'} ${ids.length} item(ns) selecionado(s)?`)) return
    setBulkLoading(true)
    setBulkMessage(null)
    try {
      ...
      if (result.failed.length > 0) {
        setBulkMessage({
          text: `${result.succeeded.length} item(ns) ${participle}, ${result.failed.length} falharam: ${result.failed.map((f) => f.reason).join('; ')}`,
          hasFailures: true,
        })
      } else {
        setBulkMessage({ text: `${result.succeeded.length} item(ns) ${participle} com sucesso.`, hasFailures: false })
      }
    } finally {
  ...
  const toggleActive = async (target: AdminItemSummary) => {
    ...
    try {
      ...
      setError(e.response?.data?.detail || 'Erro ao atualizar item')
    } finally {
      setBusy(null)
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Gerenciamento de itens</h1>
      </div>
      <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">
        Busque, ative ou desative itens da plataforma.
      </p>
      ...
      {loading ? (
        ...
        <EmptyState icon={Package} title="Nenhum item encontrado" description="Tente ajustar sua busca." />
      ) : (
        <>
              ...
              <tbody>
                {items.map((it) => (
                  <tr key={it.id} className="border-b border-gray-50 dark:border-gray-700/50 last:border-0">
                    ...
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{formatDate(it.created_at)}</td>
                    <td className="px-4 py-3">
                      <Badge variant={it.is_active ? 'green' : 'red'}>{it.is_active ? 'Ativo' : 'Desativado'}</Badge>
                    </td>
                    <td className="px-4 py-3 text-right">
                      ...
                      >
                        {it.is_active ? <><Ban className="w-3.5 h-3.5" /> Desativar</> : <><CheckCircle2 className="w-3.5 h-3.5" /> Ativar</>}
                      </Button>
                    </td>
                  </tr>
                ))}
