...
export default function JoinGroupPage() {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      setError(e.response?.data?.detail || 'Convite inválido')
    } finally {
      setJoining(false)
    }
  }
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-16 text-center">
      ...
      <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">Você foi convidado pra um grupo</h1>
      <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">
        Entre pra ver e compartilhar itens só com quem está nesse grupo no Lendly.
      </p>
      ...
      {!isLoading && !isAuthenticated ? (
        <Link href={`/login?redirect=/groups/join/${code}`}>
          <Button className="w-full" size="lg">Entrar para aceitar o convite</Button>
        </Link>
      ) : (
