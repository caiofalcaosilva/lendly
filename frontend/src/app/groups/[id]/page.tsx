...
export default function GroupDetailPage() {
  ...
  const handleDelete = async () => {
    if (!confirm('Excluir este grupo? Os itens compartilhados só com ele voltam a ser públicos.')) return
    setBusy(true)
    try {
  ...
  const handleAdminDelete = async () => {
    if (!confirm('Excluir este grupo como admin? Os itens compartilhados só com ele voltam a ser públicos.')) return
    setBusy(true)
    try {
  ...
  const handleRemoveMember = async (memberId: string, memberName: string) => {
    if (!confirm(`Remover ${memberName} deste grupo?`)) return
    setBusy(true)
    try {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Itens do grupo</h2>
      {items.length === 0 ? (
        <EmptyState
          icon={Package}
          title="Nenhum item compartilhado ainda"
          description="Ao cadastrar ou editar um item, marque este grupo em 'Visibilidade' pra compartilhar aqui."
        />
      ) : (
