...
const CATEGORIES = [
  { icon: Wrench, key: 'tools', value: 'tools' },
  { icon: Bike, key: 'sports', value: 'sports' },
  { icon: BookOpen, key: 'books', value: 'books' },
  { icon: Package, key: 'more', value: '' },
] as const
      ...
      <section className="py-20 bg-gray-50 dark:bg-gray-800">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {CATEGORIES.map(({ icon: Icon, key, value }) => (
              <Link
                key={key}
              ...
              >
                ...
                <span className="font-medium text-gray-800 dark:text-gray-200 text-sm">{t(`categories.${key}`)}</span>
              </Link>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            {FEATURE_ITEMS.map(({ icon: Icon, key }) => (
              <div key={key}>
                ...
                <h3 className="text-lg font-semibold mb-2 text-gray-900 dark:text-gray-100">{t(`features.${key}.title`)}</h3>
                <p className="text-gray-600 dark:text-gray-400 text-sm">{t(`features.${key}.desc`)}</p>
              </div>
            ))}
          </div>
        </div>
