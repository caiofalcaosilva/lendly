...
export default function GoogleConnectSection() {
  ...
  const t = useTranslations('Common.GoogleConnectSection')
  const toast = useToast()

  useEffect(() => {
    authService.getGoogleConnectStatus()
  ...
  const connect = async () => {
    setRedirecting(true)
    try {
      const { authorization_url } = await authService.getGoogleConnectUrl()
    ...
    <div className="p-4 rounded-panel border border-border bg-surface-2">
      <div className="flex items-start gap-3 mb-1">
        {connected ? (
          ...
          <GoogleIcon className="w-5 h-5 mt-0.5 flex-shrink-0" />
        )}
