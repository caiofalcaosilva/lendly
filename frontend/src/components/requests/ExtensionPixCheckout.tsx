...
// Same polling shape as PixCheckout.tsx, pointed at the extension charge
// instead of the rental one — the two are separate Payment documents (see
// docs/pagamento-online.md), so they need their own fetch/poll.
const POLL_INTERVAL_MS = 4000

export default function ExtensionPixCheckout({ requestId, onConfirmed }: Props) {
  ...
  useEffect(() => {
    const load = () => paymentsService.getExtensionPaymentForRequest(requestId).then((p) => {
      setPayment(p)
