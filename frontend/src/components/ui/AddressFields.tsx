  ...
  const lookup = useCallback(
    async (raw: string) => {
      ...
      try {
        ...
        if (data.logradouro) setValue('street', data.logradouro, { shouldDirty: true })
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
          ...
          <Controller
            ...
            render={({ field }) => (
              <input
                ...
                className={`w-full border rounded-lg px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 transition ${
                  errMsg('zip_code') ? 'border-red-400 bg-red-50' : 'border-gray-300'
                }`}
              />
            )}
          />
        ...
        {status === 'not_found' && (
          <p className="text-xs text-red-500 mt-1">CEP não encontrado. Preencha os campos manualmente.</p>
        )}
        {status === 'error' && (
          <p className="text-xs text-amber-600 mt-1">Erro ao consultar o CEP. Preencha os campos manualmente.</p>
        )}
        {errMsg('zip_code') && <p className="text-xs text-red-600 mt-1">{errMsg('zip_code')}</p>}
      </div>
